...
%token COMMENT
%%
Prog			:  		{InicTabSimb (); InicCodIntermed (); numtemp = 0;}	PROGRAM {printf("program ");}  ID {printf("%s",yylval.string); simb = InsereSimb (yylval.string, IDPROG, NAOVAR);
...
Header			:   	MAIN {printf("main");}  |   Type  ID {tid=IDVAR_FUNC; printf("%s",yylval.string); if(ProcuraSimb(yylval.string, tid) != NULL) DeclaracaoRepetida(yylval.string); else 
	{ 
...
Factor		:   	Variable {if(yylval.simb != NULL) {
